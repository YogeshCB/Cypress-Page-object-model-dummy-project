...
const getOAuthUrl = (req, addNetworkQuery = false) => {
  ...
  if (addNetworkQuery) {
    searchParams.append('network', 'shutterstock');
    redirectUrl.search = searchParams.toString();
  }
  return `${root}/api/assets/networks/shutterstock/oauth/callback?redirect=${encodeURIComponent(URL.format(redirectUrl))}`;
};
...
const handleCallback = async (req, res, next) => {
  ...
  try {
    ...
    await services.shutterstock.callback().send({
      ...req._sessionData,
      url,
    ...
    logger.error(ex);
    res.redirect(getRedirectUrl(req, 'SHUTTERSTOCK_CALLBACK_FAILED'));
  }
};
...
Router.get('/callback', [
  handleCallback,
  updateProfile('asset', 'shutterstock', {
    isConnected: true,
  }, (req, res, ex) => {
    logger.error(ex);
    res.redirect(getRedirectUrl(req, 'SHUTTERSTOCK_CALLBACK_FAILED'));
