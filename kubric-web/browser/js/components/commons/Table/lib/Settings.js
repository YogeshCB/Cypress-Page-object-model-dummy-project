...
export class GridSettings extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      }).filter(obj => typeof obj.displayName !== 'undefined')
    }
  }

  onSettingChange(index, checked) {
    ...
    }, () => this.props.onSettingChange(this.state.headers));
  }
  ...
  render() {
    ...
    return (
      <div className={`${styles.settingContainer}`} ref={node => this.node = node}>
        <Menu keepOpen={true}
              iconElement={<span className={`${theme.settings} ${styles.settings} ${open ? styles.selected : ''}`}
        ...
        </span>} orientation="bottom-right">
          {headers.map((header, index) => (
            <MenuItem>
              <Checkbox checked={header.checked} onChange={this.onSettingChange.bind(this, index)}
